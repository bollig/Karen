...
$\vx_j$, %$u(\vx_j)$ 
is approximated by a weighted combination of function values, $\{u(\vx_i)\}_{i=1}^{n}$, in a small neighborhood of \eb{$u(\vx_j)$}{$\vx_j$}{1.7}, 
where $n\ll N$ defines the size of the stencil. 
%\eb{}{(added constant term to equation:)}{}
\begin{align}
...
\ge{$r=\vectornorm{x - x_i}$}{$r=\vectornorm{\vx - \vx_i}$}{1.8}
being the distance between where the RBF is centered and where it is evaluated as measured in the standard Euclidean 2-norm. \eb{It has also been shown through experience and}{Various studies show}{}  \cite{WrightFornberg06,FornbergDriscoll02,FornbergLehto11,FlyerLehto11} that better accuracy is achieved when the 
interpolant can exactly reproduce \eb{a constant.}{a constant, $p_0$.}{1.9}  
\eb{Hence, the constraint $\sum_{i=1}^{n}w_k=\diffop{1}|_{\vx=\vx_{j}}=0$ is added, where $w_{n+1}$ is ignored once the matrix in ({\ref{syst}}) is inverted. That is,}{Assuming $p_0 = 1$, the constraint $\sum_{i=1}^{n}w_i=\diffop{1}|_{\vx=\vx_{j}}=0$ completes the system: }{1.9}
%\ge{}{Address how the constraint is added. $w_{n+1}$ is mentioned without being defined.}{1.9}
%\eb{}{($\epsilon$ has been moved outside the norm on the RHS)}{1.10}
\eb{}{\begin{align}
\begin{pmatrix}
...
\label{syst}
\end{align}
where $w_{n+1}$ is ignored after the matrix in ({\ref{syst}}) is inverted.}{1.9}
%\ge{}{Weights should be either $c_i$ or $w_i$ unless there is a reason for 
%different names in different areas of the paper}{1.11}\eb{}{(Changed all weights from `c' to `w' for consistency.)}{1.11}
%\ge{}{Address question 2.7 of reviewer}{2.7}
This \eb{}{$n \times n$}{2.7} system solve is repeated \eb{$N$ times for each}{for each stencil center }{2.7} $\vx_j$, $j=1...N$, to form \eb{}{the $N$ rows of}{2.7} the DM \eb{}{ with $n$ non-zeros $(n\ll N)$ per row}{2.7}.
As an example, if $\diffop$ is the identity operator, 
then the above procedure leads to RBF-FD interpolation. If $\diffop=\pd{}{x}$, one obtains the DM that approximates the first derivative in $x$. \eb{The reader should note that in}{In the}{} context of time-dependent PDEs, \eb{so long as the nodes do not move,}{}{} the stencil weights remain constant for all time-steps \eb{}{when the nodes are stationary}{}. Therefore, the calculation of the 
differentiation weights is performed once in a single preprocessing step of O$(n^3N)$ 
%\ge{}{floating point operations (FLOPs)}{1.12}. 
\eb{}{FLOPs.}{already defined above}
...
%\ge{}{reply to the LSH question from the reviewer involving parallelization of the search process for each point, and the issue regarding embarrassingly parallel process. Also note: this is done only once so efficiency is not an issue, nor is parallelization.}{3.1}

For each of the $N$ \eb{iterations}{small system solves}{} of Equation~(\ref{syst}), the $n$ nearest neighbors to $\vx_j$ need to be located. This can be done efficiently using neighbor query algorithms or spatial partitioning data-structures such as Locality Sensitive Hashing (LSH) and $k$D-Tree. Different query algorithms often have a profound impact on the DM structure and memory access patterns. We choose a Raster ($ijk$) ordering LSH algorithm \cite{Bollig2011} leading to the matrix structure in Figures~\ref{fig:oneThreadPerStencil} and \ref{fig:oneWarpPerStencil}. \eb{}{While querying neighbors for each stencil is an embarrassingly parallel operation, the node sets used here are stationary and require stencil generation only once. Efficiency and parallelism for this task has little impact on the overall run-time of tests, which is dominated by the time-stepping. We preprocess node sets and generate stencils serially, then load stencils and nodes from disk at run-time. In contrast to the RBF-FD view of a static grid, Lagrangian/particle based PDE algorithms promote efficient parallel variants of LSH in order to accelerate querying neighbors at each time-step \cite{Pan2011, Goswami2010}. }{3.1}

\subsection{Hyperviscosity}

For RBF-FD, \eb{the DMs are}{differentiation matrices encode convective operators of the form 
\begin{equation}
D = \alpha \pd{}{\lambda} + \beta \pd{}{\theta} \label{eqconv}
\end{equation}}{1.13}where $\alpha$ and $\beta$ are a function of the fluid velocity. The convective operator, discretized
...
%\eb{for hyperbolic equations since they have no inherent damping}{}{1.13}. 
%\ge{}{Shorter previous sentence. What diff operator is being discretized by DM?}{1.13}
Stabilization of the RBF-FD method is achieved through the application of a hyperviscosity filter 
to Equation~(\ref{eqconv}) \cite{FornbergLehto11}. By \ge{using of Gaussian}{using Gaussian}{1.14} 
...
when working on the surface of the sphere since a local stencil can be viewed as \eb{}{lying on}{1.15} a plane.
...
\ge{}{Although it is clear that when the diameter of the stencil is not sufficiently 
small compared to the radius of the sphere, that the hyperviscosity might not work as advertised,
numerical evidence suggests that if the RBFs adequately resolve the convective operator, the 2D 
...
%If a resolution of $N$ nodes is adequate at one sphere radius, it will be adequate for all radii. 
}{}
%\eb{}{A 2D approximation for the hyperviscosity suffices because its only role is to suppress spurious unphysical modes. The scale of hyperviscosity, on the order of $10^{-30}$, makes this a completely harmless assumption even in the case when the stencil radius is large relative to the size of the sphere.}{2.8}

%\ge{}{I do not understand question 2.8}{2.8}
...
\end{equation}
%\ge{}{Address the issue of continuous versus discrete form. Use words operator and matrix properly.}{2.9} \authnote{Double check}
where $D$ is the \eb{ DM operator for the PDE}{PDE operator}{2.9}, and $H$ is the hyperviscosity \eb{operator matrix}{filter operator}{2.9}.
