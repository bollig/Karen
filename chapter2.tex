...
\subsection{Sphere Surface Laplacian ($\LaplaceBeltrami$)}

Most models treat the Earth as if it is a sphere. Our tests are scaled to the unit sphere. 

For PDEs solved on the surface of the sphere, the $\Laplacian$ operator (Euclidean Laplacian) can represented in spherical polar coordinates for $\mathbb{R}^3$: 
\begin{align} 
...
\end{align}
where $\LaplaceBeltrami$ is the Laplace-Beltrami operator---i.e., the Laplacian constrained to the surface of the sphere. This form nicely illustrates the components of the $\Laplacian$ corresponding to the radial and angular terms. 

Noting that our geometry is the periphery of the unit sphere, and there is no radial term, we have:
\begin{align}
\Laplacian    \equiv \LaplaceBeltrami , 
\end{align}
and although this originated in the spherical coordinate system, we can use the following RBF operator from \cite{WrightFlyerYuen10}---Equation (20): 
\begin{align} 
...
Additionally following \cite{FlyerWright09, FlyerLehto11}, the off-diagonal blocks in Equation~\ref{eq:stokes_linear_system} corresponding to the components of a gradient operator must also be constrained to the sphere with this projection matrix: 
%\frac{1}{||\mathbf{x}||}
\begin{align}
