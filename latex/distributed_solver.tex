...
In what follows the MPI\_Alltoallv implementation is taken as baseline for communication. An attempt is then made to further communication times and improve the scaling of our implementation in a distributed CPU environment. By focusing on the CPU-only environment we can test scaling on moderately large $p$ (we go up to $p = 1024$). Improvements for distributed CPU scaling directly improve scaling of the distributed GPU implementation (see Chapter~\ref{chap:multigpu_rbffd}). 

\section{Scaling Results} 
\label{sec:cpu_scaling}


%TODO: Itasca scaling to demonstrate behavior as 
The scalability of our implementation is tested in distributed CPU mode  on the Itasca HPC cluster at the University of Minnesota. Owned and operated by the Minnesota Supercomputing Institute, the Itasca cluster is \authnote{finish}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{performance_content/scaling/strong_scaling_4M_compare_SpMV_and_comm_n50.png}
...
%\begin{figure}
%\centering
%\includegraphics[width=0.6\textwidth]{performance_content/scaling/weak_scaling_np4000_overlap_cpu_SpMV_and_comm_all_stencils.png}
