...
With a partitioning and local ordering complete, the next task is to establish communication collectives for transferring data between MPI processes. Processes commonly share data with more than one neighbor. To accommodate this, general communication collectives are assumed for many-to-many and subset-to-subset connections. 
...
\end{align}
where $T_s$ and $T_p$ are the time spent in serial and parallelizable portions of code respectively. Scaling to a very large number of processors may make the time in parallelizable portions of code vanish, but $T_s$ remains constant. In any distributed implementation communication is always included in estimate for $T_s$, so it is pertinent to choose an MPI collective that sends/receives the minimum number of bytes and requires . 
...
Our implementation in \cite{BolligFlyerErlebacher2012} leveraged a horribly inefficient send/recv collective. 
