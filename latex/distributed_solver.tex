...
Greater than 4x speedup (bold-faced in Table~\ref{tbl:cascade_m2070_overlap}) is possible due to a design flaw on Cascade: each compute node has four attached GPUs connected to the host via shared I/O hub. Concurrent data transfers from multiple GPUs to host, or vice versa---as would occur while executing a synchronized, distributed SpMV---result in contention on the hub and serialization of transfers. The serialization effectively quadruples the cost in the case of 4 PPN. As an example, consider the communication time for the non-overlapping test case (including encoding the send buffer, executing the MPI collective and decoding data), which takes approximately 80 ms. The communication time for the same test case drops by a factor $\gt 4x$ to 18ms when overlapping communication and computation is employed. 
