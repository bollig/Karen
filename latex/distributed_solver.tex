...
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{../figures/omnigraffle/GPU_Alltoallv_nooverlap.pdf}
\caption{GPU and MPI\_Alltoallv (Non-Overlapping)}
\label{fig:alltoallv_nooverlap_gpu}
\end{subfigure}
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{../figures/omnigraffle/GPU_OverlapGPU.pdf}
\caption{GPU and MPI\_Isend/MPI\_Irecv}
...
By comparing benchmarks from no overlap to the overlapped GPU case, I get speedup in using the overlapped solution. Any reduction in time is evidence of overlap. Calculating the percentage reduction is useful to consider. The benchmarks are not complete (i.e., the actual time to compute SpMV1 is unknown). However, with the expected un-overlap time we know the expected cost of communication and Can calculate the % reduction based on two samples  
