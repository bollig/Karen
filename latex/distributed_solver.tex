...
Greater than 4x speedup (bold-faced in Table~\ref{tbl:cascade_m2070_overlap}) is possible due to a design flaw on Cascade: each compute node has four attached GPUs connected to the host via shared I/O hub. Concurrent data transfers from multiple GPUs to host, or vice versa---as would occur while executing a synchronized, distributed SpMV---result in contention on the hub and serialization of transfers. Serialization effectively quadruples the total cost in the case of 4 PPN. As an example, consider the time \emph{not} spent computing the SpMV for $n=50$, and one compute node with 4 PPN. The non-overlapping test case, takes approximately 80 ms. Of that, half the time is spent in communication

The non-SpMV component includes encoding the send buffer, executing the MPI collective and GPU<->CPU transfers. 
The communication time for the same test case drops by a factor $> 4x$ to 18ms when overlapping communication and computation is employed. 

The time required to compute the SpMV for 1 node, 4 PPN without overlap (i.e. without splitting $\setCenters \backslash \setBoundary$ and $\setBoundary$) is in the neighborhood of 50 ms. 


Within each block of Table~\ref{tbl:cascade_m2070_overlap} we expect the GFLOP/sec to double by row. Obviously the strong scaling in the case of the GPU is not ideal, 
