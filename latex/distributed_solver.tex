...
\end{figure}
 ...
 Based on Figure~\ref{fig:weak_scaling_comm_only_alltoallv_all_stencils} the MPI\_Alltoallv weak scaling is poor. As $p$ grows, the total time grows at a super-linear rate. Figure~\ref{fig:weak_scaling_spmv_only_alltoallv_all_stencils}, which shows the SpMV time only, verifies that beyond the initial growth in time on a single compute node the SpMV is constant for all stencil sizes. That then implies the cost of communication must be reduced (or hidden). 
...
Figure~\ref{fig:mpi_tuning} illustrates a progression of collectives tested in an effort to improve weak scaling. 
...
A comparison of weak scaling performance for the sequence of on $n=50$ 
