...
The blocking version of this all-to-subset was employed in \cite{BolligFlyerErlebacher2012} for debugging purposes (see Appendix~\ref{app:keeneland_spear_scaling} for scaling data). All sends and receives were serialized making it infeasible to target large $p$ (tests were for $p \leq 10$). In the data that follows non-blocking sends and receives are utilized for concurrent communication with multiple processors.
...
Effort was given to tuning the communication for improved scaling. The progression occurred as follows:  
\begin{enumerate} 
\item The entire SpMV for $\setCenters$ is executed, followed by a blocking MPI\_Alltoallv collective.
\item The MPI\_Alltoallv is replaced by non-blocking MPI\_Isend/MPI\_Irecv.
\item MPI\_Irecv is issued \emph{before} starting the SpMV to ensure MPI\_Isends have pending connections when ready. 
\item By assuming the elements of $\setDepend$ are sorted contiguous by neighboring process, the entire decode process is skipped. 
\item MPI\_Irecv and MPI\_Isend are issued before computing the SpMV for set $\setCenters \backslash \setBoundary$. The process waits for communication to finish before launching the second SpMV for $\setBoundary$. This hides the cost of communication behind computation. 
\end{enumerate}
...
\label{fig:alltoallv_cpu}
\end{subfigure}
\caption{MPI tuning steps to scale from 10 processors in \cite{BolligFlyerErlebacher2012} to 1024 processors. Each horizontal timeline indicates the order of operations for a single distributed SpMV (i.e., between vertical bars). Concurrent operations are launched from top to bottom. } 
\end{figure}
...
\item Processor load balance is not 
\end{itemize}
