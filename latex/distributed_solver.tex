...
Calculating the percentage reduction is useful to consider. The benchmarks are not complete (i.e., the actual time to compute SpMV1 is unknown). However, the time spent in non-SpMV related tasks (e.g., data transfer, encoding, and communication) is known from the unoverlapped case. Therefore, given the time for the overlapped SpMV case, we can calculate the amount of overlap by 

\begin{align}
t_{overlap} = t_{overlap} - (t_{comm} + t_{en} + t_{R} + t_{O}) \\
t_{nooverlap, SpMV} - (t_{overlap, tot} - t_{overlap, non-SpMV})
\end{align}
