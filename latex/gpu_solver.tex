...
Before launching into the discussion of how GPU kernels are actually implemented for RBF-FD, we first attempt to manage expectations of the type of performance the method can achieve on the GPU. To this end we build a \emph{roofline model} \cite{Williams2009}, which is useful for expressing the maximum possible performance (in terms of GFLOP/sec) for any kernel on the GPU. 

This work operates on two generations of NVidia GPUs shown in Table~\ref{tbl:gpu_comparison}. The first is the previous generation NVidia M2070 ``Fermi"-class GPU with 6 GB global memory and a maximum memory bandwidth of 148 GB/sec \cite{M2070FactSheet}. The second GPU is the latest generation NVidia K20 ``Kepler"-class GPU, which has only 5 GB of global memory but 208 GB/sec bandwidth \cite{KeplerFactSheet}. Both GPUs are mounted on the Cascade GPU cluster at the University of Minnesota Supercomputing Institute. 

Cascade is a heterogenous compute cluster built with a total of 15 compute nodes. The first eight nodes each have two 6-core Intel Xeon X5675 ``Westmere-EP" class CPUs (3.06 GHz), 96 GB of memory and four M2070 GPUs (i.e., 32 total Fermi-class GPUs). Four compute nodes each contain dual 8-core Intel Xeon E5-2670 ``SandyBridge" class CPUs (2.60 GHz), 124 GB of memory, and two K20 GPUs (i.e., 8 total). The remaining nodes in the cluster are the same as the K20 compute nodes, except the K20 GPUs have been replaced an Intel Xeon Phi \cite{IntelXeonPhi2013}.
...
The roofline model depends on easily accessible hardware specifications.

The roofline model states
\begin{align*}
\text{Max Attainable GFLOP/sec} = \min \begin{cases} \text{DP Peak Performance} \\ \text{Max Memory Bandwidth} \times \text{Operational Intensity}  \end{cases}
\end{align*}
...
We consider the \emph{operational intensity} of a kernel, defined as the ratio of floating point operations to bytes loaded from memory. 
