...
\item \emph{Compressed Row Storage (CSR)} -- Similar to COO, the matrix is stored in three arrays. All $N*n$ nonzero weight are stored in the first array. The second array stores $N$ integers that indicate the offset (into the first array) to get to the first nonzero of each row. Finally, the $N*n$ column indices for each nonzero are stored in the third array. This format requires a total of $(2*(n+1)*N)$ storage.
\item \emph{ELLPACK (ELL)} -- The matrix is assumed to have exactly $m$ nonzeros per row. Then the matrix is stored in two arrays, each of size $N*m$, containing the matrix nonzeros and column indices. Due to our assumption that all RBF-FD stencils will have equal size, $n$, this requires a total of $(2*N)$ storage. 
\end{itemize}
