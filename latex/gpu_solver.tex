...
%In general, sparse matrices have nonzeros randomly scattered throughout each row. A structured matrix has nonzeros distributed at regular intervals with neighboring rows. 
In general nonzeros in a row can be regularly or irregularly spaced at intervals, but are rarely all consecutive. When the sparse matrix is compressed into CSR format, the packed vector with consecutive nonzeros for each row is easily loaded from memory in a few transactions and then operated on sequentially. Unfortunately, the weights must still be multiplied against the proper elements of the solution vector. Operating sequentially on the packed weights implies that the elements of the solution vector are loaded following the original (potentially irregular) spacing between nonzeros in the uncompressed sparse matrix. From the perspective of a single row the solution vector is random access. It is not uncommon for the L1 cache to pull in a 128-byte-wide cache line that contains only one or two values that will be used by a single row. %Node ordering can improve sparse structure can be improved to ensure more solution values per cache line are used by 
  % WHAT DOES THIS MEAN: treated as random access?
...
%The limited performance for ELL on the K20 problem can be caused by a number of is we know NVidia is
%providing limited support for OpenCL kernels so it is possibly a
%shortcoming of the OpenCL driver. However, the boost in performance on
%COO and CSR leads me to believe its more likely a problem of register
%density and cache utilization.
%Given the performance increased as expected for COO and CSR formats, the lack of increase for ELL may be caused by limited cache utilization. The L1 caches on the M2070 and K20 have the same size. If cache lines are underutilized (e.g., if only a few doubles per line are used), then (all else being equal) both GPUs evict lines at the same time and performance is limited by the number of cycles .  then the 8 GFLOP/sec can be improved by techniques like node ordering (see Chapter~\ref{chap:stencils}). 

% WHY IS THIS EVAN? MAKE SOME STATTEMENT IF YOU CAN. 
...
% What is the maximum bandwidth on the GPUs in practice? Did you measure? I must have missed this. 
