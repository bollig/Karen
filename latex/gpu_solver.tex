...
Each derivative operator is stored as an independent CSR matrix with independent indexing. 
...
In general nonzeros in a row can be regularly or irregularly spaced at intervals, but are rarely all consecutive. When the sparse matrix is compressed into CSR format, each row is condensed into a packed vector with consecutive nonzeros that can be easily loaded from memory in a few transactions and then operated on sequentially. Unfortunately, the packed vector must still be multiplied against the proper elements of the solution vector. Operating sequentially on the packed weights implies that the elements of the solution vector are accessed according to the original spacing between nonzeros of the uncompressed sparse matrix. As In other words, the solution vector is in general random access. 
