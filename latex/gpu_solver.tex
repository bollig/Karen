...
GPUs originated as dedicated hardware for video games and computer graphics (e.g., for rasterization). 
Due to the inherent task parallelism in computer graphics, early GPU designers adopted a many-core strategy for hardware, which persists today. Since computer graphics traditionally involved simple operations, the majority of transistors on a GPU are dedicated to computation in vector pipelines with limited control for logic. In contrast to this, CPUs have always focused on fast serial and scalar operations. Thus, CPUs allocate a smaller number to computation in order to satisfy the needs for advanced logic tasks like branching, caching, etc \cite{Owens2007,CudaGuide2013}. Figure~\ref{fig:gpu-devotes-more-transistors-to-data-processing} (from \cite{CudaGuide2013}) gives a high level perspective of CPU versus GPU architecture. A GPU has a high density of Arithmetic Logic Units (ALUs) with simple simple control and caching on shared DRAM memory. 

%TODO: verify ALU 

Thanks to the highly profitable and always demanding game industry, the static rendering pipeline of yore, was molded into a fully programmable and dynamic execution platform with a SIMD-like platform. Prior to 2006, the GPU had already evolved away from fixed-function pipelines and on to limited programmable functionality with control over a subset of phases in the rendering process. At the time researchers were able---with significant effort---to trick the GPU into solving scientific problems by encoding solutions within the graphics rendering process (see e.g., \cite{Harris2005,Owens2007}).
...
Individual cores on a GPU operate at a lower clock frequency compared to a CPU, but the sheer number of cores on a GPU results in tremendous throughput in terms of floating point operations per second (FLOPs). Figure~\ref{fig:floating-point-operations-per-second} (from \cite{CudaGuide2013}) shows the historical growth in terms of billions of FLOP/s (GFLOP/s) for NVidia GPUs versus Intel CPUs. In the last decade the peak GFLOP/s for a single GPU has sky-rocketed, leaving CPUs a full order of magnitude behind. Similarly, Figure~\ref{fig:memory-bandwidth} compares the difference in memory bandwidth between the two vendors. In order to supply much needed data to the many cores, NVidia GPUs have evolved a memory hierarchy with bandwidth up to five times higher than Intel CPUs \cite{CudaGuide2013}. 
