...
The API is structured following existing programming and interface conventions from the \ttt{Boost::uBLAS} library, a CPU-only linear algebra library (\cite{BoostSite}). In fact, the ViennaCL and \ttt{Boost::uBLAS} APIs are directly compatible, and many ViennaCL algorithms (e.g., iterative solvers, preconditioners, etc.) can be applied to \ttt{Boost::uBLAS} data structures. Similar interoperability exists between ViennaCL and the EIGEN and MTL libraries. 
