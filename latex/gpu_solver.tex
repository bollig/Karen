...
\begin{itemize} 
\item \emph{Coordinate Matrix (COO)} -- The sparse matrix is compressed to three arrays, each containing $N*n$ elements. The first array stores all nonzero weights from the matrix. The second and third arrays store the row and column indices (resp.) for each nonzero element. The COO format does not require nonzeros 
\item \emph{Compressed Row Storage (CSR)} -- This format assumes weights are sorted by row. 
