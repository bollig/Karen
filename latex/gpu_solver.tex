...
We start in Figure~\ref{fig:gflops_cascade_m2070} with the observed GFLOP/sec achieved on the M2070. For each stencil size the ELL format clearly demonstrates itself to perform the best and the format achieves up to 8 GFLOP/sec in the case of $n=101$ nodes per stencil. Unsurprisingly we find that the performance of the CSR format on the GPU is nearly the same GFLOP/sec achieved by our custom GPU kernels in Appendix~\ref{app:keeneland_alltoallv_benchmarks}.
...
Figure~\ref{fig:gflops_cascade_k20} shows the achieved GFLOP/sec for Cascade's K20 GPUs. In this case we expect the K20 to perform up to 50\% better than the M2070 due to increased bandwidth. Although an increase is visible in the case of the CSR and COO, the ELL disappointingly performs the same as the M2070 with 8 GFLOP/sec peak for stencil size $n=101$. 
...
Figures~\ref{fig:ell_gflops_cascade_m2070} and \ref{fig:ell_gflops_cascade_k20} summarize the best achieved GFLOP/sec (in all cases the ELL format) for each stencil size on the M2070 and K20 respectively. Surprisingly, for stencil sizes $n=17$ and $n=31$ we find that the K20 performs worse than the M2070. 

Figures~\ref{fig:ell_speedup_cascade_m2070} and \ref{fig:ell_speedup_cascade_k20} demonstrate the achieved speedup each GPU gains over their respective CPUs running the Boost::uBLAS CSR format (i.e., Westmere-EP and Sandy-Bridge). In \cite{BolligFlyerErlebacher2012} and Appendix~\ref{app:keeneland_alltoallv_benchmarks} our custom kernels only achieved up to 4x speedup over the CPU in setups nearly identical to the M2070 case here. Leveraging the ELL format provides between 10x and 14x acceleration on one million nodes.

In the case of the K20, Figure~\ref{fig:ell_speedup_cascade_k20} shows that the faster Sandy-Bridge architecture closes some of the divide between CPUs and GPUs, but the GPU is still between 8x to 10x faster than the CPU on one million nodes. 
...
In early 2013, Intel released a beta OpenCL driver for the Xeon Phi. In light of this we were able to repeat our ViennaCL benchmarks on the new hardware. The resulting data, presented in Figure~\ref{fig:gflops_cascade_intel_phi} \emph{should only be considered preliminary}.  The figure is provided for comparison with Figures~\ref{fig:gflops_cascade_m2070} and \ref{fig:gflops_cascade_k20}. The most obvious issue with Figure~\ref{fig:gflops_cascade_intel_phi} is that every single test case on the Xeon Phi demonstrates substantially lower performance than on NVidia GPUs. We suspect that the low performance is due in part to the beta status of the OpenCL driver. Another concern is that the ViennaCL SpMV kernels were written for NVidia hardware and it is known that OpenCL does not guarantee performance portability. 
...
Two custom GPU SpMV kernels were described: one that performs the SpMV by dedicating one thread to compute each row of the differentiation matrix, and one that applies 32 threads (a warp) to compute each row. The performance of the custom kernels is rather low at 3 GFLOP/sec at best, but recent efforts to leverage the reduced memory footprint of the ELLPACK sparse matrix format in the ViennaCL library ultimately achieves up to 8 GFLOP/sec, or 43\% of the peak possible performance for SpMV on NVidia M2070 GPUs. 

Note there is still room to improve the SpMV performance in addition to a few other directions for future work:
\begin{itemize} 
\item 
It has come to our attention that the market today appears to favor NVidia for GPU computing, with the majority of clusters world-wide containing NVidia accelerators (typically Tesla units). The CUDA C/C++ compiler was open-sourced in the Spring of 2012 as an LLVM compiler, allowing other languages to directly interface with the GPU without the need for wrappers (see e.g., \cite{NumbaPro}). As we proceed into the future effort will likely focus on CUDA kernels. 
