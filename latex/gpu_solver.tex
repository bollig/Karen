...
%DONE: sparse support was missing but it is key to a lot of problems
%DONE: added in Bell
%TODO: problem with SpMV is that it is memory bound. 
%TODO: many investigations followed within SuKeutzer, Kreutzter, Vuduc etc.
...
Unfortunately, the SpMV is an inherently memory bound problem (\cite{Bell2009, SuKeutzer2012, Kreutzer2012}). The general SpMV multiplies nonzero elements of each matrix row against corresponding elements of a dense vector. Each row is only applied once as a sparse dot product before it is discarded from memory. The sparse dot product performs two floating point operations (i.e., a multiply and add) for each element of the matrix. A major concern for SpMV is that either a random set of weights per row are multiplied against a dense, structured vector, or the set of weights for each row are condensed for structure, but then multiplied against a random-access vector. In either case, the structure of the sparse matrix (i.e., how regular or random the weights are distributed in the rows) governs how well the GPU utilizes cache and the overall performance. 

When CUDA was first released in 2006 it lacked support for sparse linear algebra. Bell and Garland \cite{Bell2009} launched the first study of the general SpMV problem on the GPU in 2009, and introduced a set of five sparse matrix data structures, or \emph{sparse formats} for the GPU, which compress the matrix in memory based on the presence of certain types of structure in the nonzeros. By embracing the structure of the matrix, the sparse formats reduce the total number of bytes required to store the matrix and thus the number of bytes that must be loaded by the memory bound SpMV kernels. 

Ultimately, the sparse formats in \cite{Bell2009} were included in the cuSPARSE library, which was added to the standard CUDA Toolkit in 2010  and provides only basic sparse matrix operations (e.g., multiplication, addition, etc.) \cite{CudaToolkitDoc}. Bell and Garland's formats are also included in the third party sparse linear algebra libraries CUSP \cite{Cusp2012} and ViennaCL \cite{Rupp2010,Rupp2010a}, which both expand beyond the primitive matrix operations to include features like dense matrix algebra, iterative Krylov solvers, preconditioners, etc. 
...
are very similar in that they  Both libraries offer similar sparse matrix containers including:    


for sparse linear algebra 

The work in \cite{Bell2009} led to a number of more recent investigations into sparse matrix handling on the GPU including \cite{SuKeutzer2012,Kreutzer2012,LiSaad2010} and resulted in third party libraries like . The CUSP and ViennaCL libraries 
...
Ultimately, Bell and Garland proposed a number of \emph{sparse formats} or data structures to fit a variety of problem types. Each sparse format 
Because the problem is inherently memory bound, optimizations focus on reducing the amount of memory that is loaded, and embracing any regularity in structure of the sparse matrix.

Of course application specific kernels had hope to achieve better performance with fine tuned logic 

The formats from \cite{Bell2009} were provided as part of the CUSP library \cite{Cusp2012} and then they were later integrated into standard CUDA Toolkit as the cuSPARSE library \cite{CudaToolkitDoc}. An OpenCL library similar to CUSP
...
%TODO: cost increases at each level of memory 
%TODO: load memory in lines of 128 bits. 
%TODO: rule of thumb to keep data close to compute threads

%TODO: sparse support was missing but it is key to a lot of problems
%TODO: added in Bell
%TODO: many investigations followed within SuKeutzer, Kreutzter, Vuduc etc.
%TODO: problem with SpMV is operational intensity 
%TODO: OI dictates GFLOP/sec (define GFLOP/sec)
