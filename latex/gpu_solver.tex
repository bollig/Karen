...
Ultimately, the sparse formats in \cite{Bell2009} were included included in the CUDA Toolkit for basic sparse matrix operations (e.g., multiplication, addition, etc.) \cite{CudaToolkitDoc}. The same formats are also included in the third party sparse linear algebra libraries CUSP \cite{Cusp2012} and ViennaCL \cite{Rupp2010,Rupp2010a}, which expand beyond the basic matrix operations to include features like dense matrix algebra, iterative Krylov solvers, preconditioners, etc. 



Our early efforts to port RBF-FD onto the GPU (\cite{BolligFlyerErlebacher2012}) focused on the implementation of a custom set of OpenCL kernels. The kernels were developed to apply RBF-FD weights in the equivalent of multiple SpMV operations.
...
%DONE: cost increases at each level of memory 
%DONE: load memory in lines of 128 bits. 
%DONE: rule of thumb to keep data close to compute threads

%DONE: sparse support was missing but it is key to a lot of problems
%DONE: added in Bell
%DONE: problem with SpMV is that it is memory bound. 
%DONE: many investigations followed within SuKeutzer, Kreutzter, Vuduc etc.
...
In our initial implementation, published in \cite{BolligFlyerErlebacher2012}, all bets were hedged in favor of OpenCL as the future of GPU computing. Custom  
