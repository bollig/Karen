...
This appendix provides new data from Keeneland for direct comparison with \cite{BolligFlyerErlebacher2012}. All data in this Appendix was acquired prior to the 2012 upgrade from the Keeneland Initial Delivery System (KIDS) to the Keeneland Full Scale system (KFS). As such, benchmarks were gathered on NVidia M2070 GPUs (150 GByte/sec bandwidth), rather than the newer M2090 GPUs (177 GByte/sec bandwidth) \cite{M2090}. 
...
This section considers the performance of a full RK4 iteration for the Cosine Bell Advection test case in \S~\ref{sec:cosine_bell}. As discussed in Chapter~\ref{chap:gpu_rbffd}, a regular SpMV involves $(2Nn)$ FLOPs. Intermediate steps of the RK4 (i.e., the evaluations of $F(t,\vu)$) perform three SpMV operations: one for each $D_{\theta}$, $D_{\lambda}$ and $D_{hv}$, plus four vector operations to join SpMV results and prepare the input for the next intermediate step. The cost of an intermediate step is therefore estimated at $(6Nn + 4N)$ FLOPs. In that case, the Cosine Bell RK4 iteration with four intermediate steps plus another $5N$ FLOP overhead to combine intermediate solutions totals to approximately $(24Nn + 23N)$ FLOPs. 

Figures~\ref{fig:gflops_cpu_1proc_keeneland}, \ref{fig:gflops_gpu_1proc_oneWarp_keeneland} and \ref{fig:gflops_gpu_1proc_oneThread_keeneland}, each function evaluation in the RK4 performs three independent SpMVs and two AXPY operations ($2*N$ each) for a total of $6*N*n + 4*N$ operations. Thus the total operation count for a single RK4 iteration is estimated at: $24*N*n +23*N$. 
...
\end{subfigure} 
\caption{Cosine Bell Advection benchmarks from the Keeneland GPU cluster compare the average run-time of a single time-step of RK4 with three SpMVs per intermediate time-step evaluation.}
\end{figure} 
...
\section{Vortex Roll-Up on Keeneland} 
...
\label{fig:speedup_1proc_oneThread_keeneland}
\end{subfigure} 
\caption{Vortex Roll-up benchmarks from Keeneland compare the average run-time of a single time-step of RK4 with two SpMVs per intermediate time-step.}
\end{figure} 
