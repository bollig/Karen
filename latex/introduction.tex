...
In what follows, we classify a number of related works according to the type of parallelism they exhibit. This section follows closely with Table~\ref{tbl:parallel_rbf_refs}
...
\begin{table}
    ...
    \begin{tabular}{L{0.125\textwidth}|C{0.09\textwidth}|C{0.09\textwidth}|C{0.09\textwidth}|C{0.09\textwidth}|C{0.09\textwidth}|C{0.09\textwidth}|C{0.09\textwidth}|}
         % Use a star or cross to denote OpenMP + MPI. 
                                    & \multicolumn{2}{C{0.2\textwidth}|}{Multi-CPU} & \multicolumn{3}{C{0.3\textwidth}|}{Single GPU} & \multicolumn{2}{C{0.2\textwidth}|}{Multi-GPU ($\star$:Non-overlapped, $\dag$:Overlapped)} \\ \cline{2-8}
                                    & OpenMP     & MPI     & CUDA     & OpenCL     & Other     & CUDA     & OpenCL   \\ \hline
             RBF-FD                 &            &         &          &            &            &          & \cite{BolligFlyerErlebacher2012}$^{\star,\dag}$  \\ \hline
             RBF PDE Methods  &  \cite{Kosec2008}   & \cite{Divo2007,Yokota2010} & \cite{Schmidt2009a} &    &   &   &  \\ \hline
             Non-PDE RBF Methods    &             &        &  \cite{Cuomo2013,Cuntz2007}  &   &  \cite{Corrigan2005,Weiler2005,Carr2003,Brandstetter2008}  &      &      \\ \hline
             Non-RBF PDE Methods    &     &       &       &      &      &     &  \\ \hline
    \end{tabular}
\end{table}
...
\item A CPU-only ILU0 preconditioner is implemented to test impact on GMRES convergence rates 
\end{itemize}
