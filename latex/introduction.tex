...
\begin{table}
    ...
    \begin{tabular}{L{0.11\textwidth}||C{0.1\textwidth}|C{0.08\textwidth}||C{0.08\textwidth}|C{0.09\textwidth}|C{0.08\textwidth}||C{0.1\textwidth}|C{0.125\textwidth}||}
         % Use a star or cross to denote OpenMP + MPI. 
             ...
             RBF-FD                 &            &         &          &            &            &          & \cite{BolligFlyerErlebacher2012}$^{\dag}$ \mbox{[This Work]$^{\ddag}$}  \\ 		\hdashline
             RBF PDE Methods  &  \cite{Kosec2008}   & \cite{Divo2005, Divo2007, Ingber2003,Ingber2004,Yokota2010} & \cite{Schmidt2009a,Schmidt2009b,Gumerov2007a, Gumerov2007b} &    &   &   &  \\ 		\hdashline
...
While a number of related works consider RBFs on GPUs, most of the investigations relate to visualization (e.g., \cite{Cuntz2007, Weiler2005}),  surface reconstruction (e.g., \cite{Corrigan2005,Carr2003}), and neural networks (e.g., \cite{Brandstetter2008}). Of these, only \cite{Cuntz2007} uses a modern GPU langauge (CUDA), the rest rely on shader languages (i.e., GLSL, HLSL, etc) denoted as ``Other" in Table~\ref{tbl:parallel_rbf_refs}.
...
Thibault et al. \cite{Thibault2009} also note the need for overlap when modeling 3-D incompressible fluids with finite differences on structured grids and a Jacobi iteration. %Their solutions to the lid-driven cavity problem are based on
...
%The bulk of work for the RBF-FD method is a Sparse Matrix Vector (SpMV) Multiplication. Efforts such as \cite{Schubert2011,Kreutzer2012} investigate overlapping communication and computation techniques to distribute 
...
\item The fixed-grid method is also shown to accelerate RBF-FD time-step performance by up to 5x due to spatial locality of node information in memory and improved cache effects.
\end{itemize} 
...
\begin{itemize} 
\item A comparison of the variants to the result of the well-known Reverse Cuthill-McKee (RCM) algorithm finds that RCM is the better option for minimizing the matrix bandwidth. 
%GORDON: Given a grid of 48x48x96 or 96x48x48 or 48x96x48: RCM will have different effects with 48x48x96 producing the lowest bandwidth. You know why? (at least I think that is the case.)
%Response: 
\end{itemize} 
...
\begin{itemize} 
\item Our home-grown implementation distributes computation with a Restricted Additive Schwarz (RAS) domain decomposition in the first application to RBF-FD. % (note: other RBF methods have already encountered RAS decomposition \cite{Yokota2010}). 
...
\item Scaling benchmarks (\S~\ref{sec:scaling_summary}) up to 1024 processes (divided into 8 processes per node) and a grid resolution of $N=160^3$ vertices (i.e., 4.1 million) demonstrate that the implementation scales moderately well in both a strong and weak sense. 
\end{itemize} 
