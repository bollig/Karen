...
Our current application of interleaving was started only to simplify MPI communication and data transfers between CPUs and GPUs. However, future work will continue to investigate interleaving for the purpose of increased operational intensity. Each of the 16 blocks of Equation~\ref{eq:stokes_constant_viscosity} can be multiplied independently against the four vectors $U, V, W$ and $p$. Our work in \cite{ErlebacherSauleFlyerBollig2013} shows that multiplying multiple sparse matrices with the same sparsity pattern against multiple vectors---that is, performing multiple Sparse Matrix-Matrix multiplies (SpMMs)---is equivalent to a tensor cross product ($4\times1$ times $1\times4$) for each nonzero element of the matrices. The tensor cross products 

increases operational intensity (from $\frac{1}{8}$ to $1$) and register density in order to achieve significantly higher GFLOP/sec a single SpMV. 

If operating independently, each small $4\times4$ block of the interleaved system can be reproduced by a tensor cross-product with .
