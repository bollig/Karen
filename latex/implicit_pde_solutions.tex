...
In Algorithm~\ref{alg:gmres}, Lines \ref{alg:gmres_precond} and \ref{alg:gmres_precond2} depend on the application of $M^{-1}$, a preconditioner 
...
\begin{align*} 
\Laplacian = \underbrace{\frac{1}{\hat{r}} \pd{}{\hat{r}} \left( \hat{r}^{2} \pd{}{\hat{r}}  \right)}_{\mathsf{radial}} + \underbrace{\frac{1}{\hat{r}^2} \Delta_{S}}_{\mathsf{angular}}. %\label{eq:laplacian_as_radial_angular}
\end{align*}
...
Interleaved solutions can also be replaced by multiple SpMVs. For example, our work in \cite{ErlebacherSauleFlyerBollig2013} investigates the 
...
The manufactured solution is shown in Figure~\ref{fig:manufactured_solution}, projected onto the plane via a Mollweide projection: 
\begin{align*}
