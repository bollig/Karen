...
The reduced performance observed for multiple GPUs per node (e.g., 1 compute node, 4 PPN) reflects a known problem in the design of Cascade. To understand this we turn to  Each of the M2070 compute nodes on Cascade has four GPUs attached to independent PCI-E buses. Those PCI-E buses junction at an I/O Hub which is in turn connected to the a bottleneck in the path between CPU and the four GPUs.

\begin{figure}
\centering
\includegraphics[0.5\textwidth]{./gpu_content/omnigraffle/CascadeIOHub.pdf}
\caption{A high level view of the }
