...
In Appendix~\ref{app:keeneland_alltoallv_benchmarks}, we present scaling data for our multi-GPU implementation based on custom OpenCL kernels developed for \cite{BolligFlyerErlebacher2012}. The data was generated on the Keeneland Initial Delivery (KID) system \cite{Vetter2011}, and shows strong scaling up to 10 GPUs (1 GPU per compute node) applied to the small problem size of $N=27556$ nodes. Non-overlapping communication (\texttt{MPI\_Alltoallv}) synchronizes GPUs as they execute iterations of RK4 time-scheme. 
