...
Table~\ref{tbl:cascade_m2070_nonoverlap} presents three groups data (GFLOP/sec) observed for each $1, 2, 4$ and $8$ compute nodes. The first group tests the execution of one MPI process per node (i.e., 1 PPN) for each compute node. The second group scales the number of processes per node two, and the third group occupies all four GPUs per node with 4 PPN.  Data for the $n=101$, single process, single node test case could not be executed due to the limited availability of memory on the M2070 GPUs (matrix alone required $((2*N*n)*8\ \text{bytes}) = 6.6$ GB storage for the ELL matrix.
