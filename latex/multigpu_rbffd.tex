...
In order to develop the multi-GPU implementation, we return to the idealized test case considered in Chapter~\ref{chap:distributed_rbffd}: four derivatives on a 3-D regular grid are computed via SpMV and used as the intermediate vectors for an RK4 time-step. At the end of each SpMV a communication collective synchronizes external dependencies for each subdomain.

Previously, the SpMV was assumed to be on the CPU. Herein, we assume the SpMV is computed via the GPU using the algorithms presented in Chapter~\ref{chap:gpu_rbffd}. In the description of our multi-GPU algorithm the actual choice between custom OpenCL kernels and ViennaCL does not matter. However, as we proceed with benchmarking the ViennaCL implementation is assumed. 
