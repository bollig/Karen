...
Table~\ref{tbl:cascade_m2070_nonoverlap} presents three groups of data for $p=1, 2, 4$ and $8$ compute nodes. In the first group each test case jobs with 1 processes per node (PPN), with each process. 

For $n=101$ the single process, single node test could not be executed because the M2070 is limited to 6 GB global memory and the test case requires $(2*N*n)*8 \text{bytes}) = 6.6$ GB for the ELL matrix.
