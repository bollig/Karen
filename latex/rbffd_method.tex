...
\subsubsection{Regular Grid}For basic debugging and benchmarking purposes the most natural choice is to start with a regular or Cartesian grid. Equally spaced nodes in 1D, 2D or 3D are simple to generate and refinements (for convergence tests) are direct subsamples.     

\begin{itemize}
\item Benefits: besides a random grid, this is the simplest grid to generate.
\item Pitfalls: the regular grid does not take advantage of RBF-FD method's ability to function on unstructured nodes. 
\end{itemize}
