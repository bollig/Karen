...
The most limiting factor in the success of RBF methods is not the complexity of the methods, nor the task of approximating derivatives. Rather, it is the support parameter, $\epsilon$, and the dilemma one faces in the \emph{Uncertainty Relation} \cite{Schaback1995}. Recall that as $\epsilon \rightarrow 0$ ill-conditioning of the RBF interpolation matrices increases, but the accuracy also increases. Similarly, as the number of collocation points increases, so too does the ill-conditioning. Solving the the 
...
RBF-QR 
method can be implemented in less than 100 lines of Matlab code, and it allows solution of large problems that are typically ill-conditioned. Fornberg, Larsson and Flyer \cite{Fornberg2011a} successfully applied RBF-QR on large problems with 6000 quasi-unifrom nodes for globally supported basis functions. RBF-QR 
...
Most recently, Fornberg et al. \cite{Fornberg2012} introduced a method called RBF-GA, which performs a change of basis based on expansion of RBFs to express interpolation in terms of incomplete Gamma functions. The change of basis allows for an assembly

Benchmarks provided by \cite{Fornberg2012} demonstrate the cost (i.e., time to solve for weights) of existing stable methods in order from lowest to highest as: RBF-Direct, RBF-GA, RBF-QR, and Contour-Pad\'{e}. 

 limited to less than 2000 nodes. In the case of RBF-FD that number exceeds the stencil size for all known applications, so the method hold significant promise as a stable method 
...
\begin{figure}
\centering
\includegraphics[width=6.5cm]{/Users/evan/Karen/figures/stencils/computeWeights_hash_vs_kdtree_hnx100.png}
\includegraphics[width=6.5cm]{/Users/evan/Karen/figures/stencils/SpMV_hash_vs_kdtree_hnx100.png} \\
\includegraphics[width=6.5cm]{/Users/evan/Karen/figures/stencils/speedup_hash_vs_kdtree_hnx100.png}
\includegraphics[width=6.5cm]{/Users/evan/Karen/figures/stencils/hash_vs_kdtree_hnx100.png}
