...
The example 2-D RBFs presented in Figure~\ref{fig:rbfs} are infinitely smooth and satisfy the (conditional) positive definite requirements. 
...
Historically, the most common way to leverage RBFs for PDE solutions is in a global interpolation sense. That is, the value of a function or any of its derivatives 
at a node location is a linear combination of all the function values over the \textit{entire} domain, just as in a pseudospectral method. If using infinitely smooth RBFs, this can lead to spectral (exponential) convergence of the RBF interpolant for smooth data \cite{Fornberg2005}. %(need reference 
...
they easily extend into higher dimensions without significant change in programming complexity. Examples of commonly used RBFs from the literature are provided in Table~\ref{tbl:rbfs}; 2-D representations of the same functions can be found in Figure~\ref{fig:rbf_examples}. 
...
including those presented in Table~\ref{tbl:rbfs} have been applied in the literature, but Gaussian RBFs are particularly popular as of this writing due to recent advances in eigenvalue stabilization and new methods for investigating the $\epsilon \rightarrow 0$ regime (see e.g., \cite{FornbergLehto11,Fornberg2011a}). 
...
\begin{table}[t]
   ...
   \begin{tabular}{E | C{1in}| C{1in}| C{1in}| c | } % Column formatting, @{} suppresses leading/trailing space
...
Three types of collocation occur throughout the RBF literature: 
...
first performs a change of basis for the interpolant by directly applying the differential operator to the RBFs. It then collocates using the same approach as Kansa's method \cite{Stevens2009b, Larsson2003}. The resulting block structure of the linear system is symmetric and 
...
Hon et al. \cite{Hon1999} employed Kansa's method to solve the shallow water equations for Typhoon simulation.
...
results show that RBFs allow for longer time steps with spectral accuracy. The survey \cite{FlyerFornberg11} by Flyer and Fornberg showcases RBF-PS (Kansa's method) compared to some of the of the best available methods in geosciences, namely: Finite Volume, Spectral Elements, Double Fourier, and Spherical Harmonics. When applied to problems such as transport on the sphere \cite{FlyerWright07}, shallow water equations \cite{FlyerWright09}, and 3-D mantle convection \cite{WrightFlyerYuen10}, RBF-PS consistently required fewer time steps, and a fraction of the nodes for similar accuracy \cite{FlyerFornberg11}. 
...
\label{eqn:csrbf}
\end{equation}
where a cut-off radius, $r$, is defined past which the RBF (in this case $\varphi(r)$) has no influence on the interpolant. Note that the radius in Equation~\ref{eqn:csrbf} can be scaled to fit a desired support. Methods that leverage CSRBFs produce a global interpolation matrix that is \emph{sparse} and therefore results in a system that is more efficiently assembled and solved with smaller memory requirements \cite{Fasshauer2007}. The actual complexity estimate of the CSRBF method depends on the sparsity of the problem and on the ordering of the assembled system. Assuming $n \ll N$ where $n$ represents the number of nodes within the support region, \cite{Zhang2004} lists the complexity as dominated by $O(N)$ for properly structured systems, and the investigation in \cite{Morse2005} found $O(N^{1.5})$ consistent---in the range of $N$ tested---with the estimate provided by their choice of general sparse solver package (a sparse LU decomposition). A multi-level CSRBF method, introduced by Fasshauer \cite{Fasshauer2007}, collocates solutions over multiple grid refinements to achieve reduced $O(N)$ complexity, but the method is plagued by poor convergence. It is also worth noting that in the context of CSRBFs, analogues to Kansa's method and Fasshauer's method are known by the names \emph{radial point interpolation method (RPIM)} \cite{Wang2002} and \emph{radial point interpolation collocation method (RPICM)} \cite{Liu2005}, respectively. A more thorough survey of CSRBF history can be found in \cite{Fasshauer2007,Iske2004}.

CSRBFs have attracted a lot of attention in applications. For example, in the field of dynamic surface and image deformation, compact support allows for local transformations that do not induce global deformation (see e.g., \cite{Yang2008, Lin2009, Correa2007}). 
...
solved 
with less conditioning issues. In \cite{Sarler2006}, the authors consider 2-D diffusion problems. Divo and Kassab \cite{Divo2007} 
employ the 
...
Fasshauer's method called \emph{local Hermitian interpolation}. The authors have applied their method to 3-D soil 
...
\begin{equation}
	u_{\phi}(x) = \sum_{j=1}^{N}  \phi_j(x) c_{j} + \sum_{l=1}^{M}  P_l(x) d_{l}, \hskip1.5em P_l(x) \in \Pi^{d}_{m}
...
second sum represents a linear combination of polynomials that enforces zero approximation error
 when $u(x)$ is a polynomial of degree less than or equal to $m$. The variable $d$ is the 
 problem dimension (i.e., $u_{\phi}(x) \in \R^{d}$). 
%\toevan{Finish to end of paragraph} 
To eliminate degrees of freedom for well-posedness, $m$ should be greater than or equal to the order of the chosen RBF
 (see Table~\ref{tbl:rbfs}; more on this in \S~\ref{sec:rbf_interp}) \cite{Iske2004}.  
Note that Equation~\ref{eqn:pde_approx} is evaluated 
 at $\{x_j\}_{j=1}^{N}$ 
data points through which the interpolant is required to pass with zero residual.  The $x_j$'s are known as \emph{collocation points} (a.k.a. trial points), taken as the RBF centers. The test points, $x$, usually coincide with collocation points, $x_j$, although this is not a requirement. 
...
\end{eqnarray*}
The invertibility of this system depends on the choice of RBF, so one typically chooses a function that leads to a positive definite interpolation matrix to avoid issues. It has been shown (see \cite{Fasshauer2007, Iske2004}) that some choices of RBFs (e.g. multiquadrics and thin-plate splines \cite{Hon2001}) do not satisfy this positive definite requirement and therefore have no guarantee that the approximation is well-posed. A sufficient condition for well-posedness is that the matrix be \emph{conditionally positive definite}. In \cite{Fasshauer2007}, Fasshauer demonstrates that conditional positive definiteness is guaranteed when Equation~\ref{eqn:pde_approx} exactly reproduces functions of degree less than or equal to $m$. 
For RBF scattered data interpolation in one dimension, this can be achieved by adding a polynomial of order $m$ with $M = \binom{m+1}{1}$ terms (e.g., $x^0, x^1, \cdots, x^{m}$). 
%For $\R^2$, the terms would be: $1, x, y, xy, x^2y, xy^2, \cdots, x^{m}y^{m-1}, x^{m-1}y^{m}, x^my^m$. 
In $\R^d$, $M = \binom{m+d}{d}$ \cite{Iske2004}, giving
\begin{eqnarray}
...
\left[ \begin{array}{c c} 
	...
	\end{array} \right] \left( \begin{array}{c}
						 ...
						 \right) & = & \parray{c}{ f } \nonumber
\end{eqnarray}
where the second summation (referred to as \emph{interpolation conditions} \cite{Iske2004}) ensures that the interpolant has at least $m$-th order accuracy. Refer to Table~\ref{tbl:rbfs} for a short list of recommended RBFs and minimally required orders of $m$. In this document we work with the Gaussian RBF. Notice, in Equation~\ref{eqn:interpolation_constraints}, that the interpolation conditions add $M$ new degrees of freedom, so $M$ 
...
Equation~\ref{eq:solve_rbf_scattered_interp}---typically a dense system except in the case of RBFs with compact support---can be solved efficiently via standard methods like LU-decomposition.  With the coefficients, the interpolant can be sampled at any set of test points, $\{x_i\}_{i=1}^{n}$, by substitution into Equation~\ref{eq:rbf_scattered_data_interp}:
\begin{eqnarray}
...
In the next few subsections, collocation equations are considered based on the general form: 
\begin{eqnarray*}
\diffop{u_\phi(x)} &=& f(x) \on{\Interior} \\ 
\boundop{u_\phi(x)} &=& g(x) \on{\Boundary}  
\end{eqnarray*}
...
For all of the RBF methods for PDEs discussed in the remainder of this chapter, the following a linear system is generated: 
\begin{equation*}
...
\begin{equation*}
 ...
 \end{equation*}
 where the matrix $A_{\diffop{}}$ depends on the choice of collocation method. 
...
\begin{eqnarray}
u(x) & \approx &  \left. \left[ \begin{array}{c c} 
       \phi &  P
	\end{array} \right]
	  \left( \begin{array}{cc}  c \\ d  \end{array} \right)  \ \right|_{x={x_i}} \nonumber \\
...
\sum_{j=1}^{N}c_j\boundop{\phi_j(x_i)} + \sum_{l=1}^{M}d_l \boundop{P_l(x_i)} &=& g(x_i)  \hskip1.5em i = N_I+1,...,N \label{eqn:direct_boundary} \nonumber \\
...
& = & D^T_{\diffop{}} \left( \begin{array}{cc}  f \\ 0  \end{array} \right). 
\end{eqnarray*}
...
\end{eqnarray*}
An LU-decomposition ($O(N^3)$) in preprocessing with forward- and back-solves ($O(N^2)$) are appropriate to efficiently solve the multiple RHS system\cite{WrightFlyerYuen10,Fasshauer2007}. 

Since matrix $D_{\diffop{}}$ is independent of functions $u(x)$ and $f(x)$, the matrix requires an update only if the RBF centers move---a compelling benefit for time-dependent problems on stationary nodes. The complexity of RBF-PS for time-dependent solutions is then reduced to a matrix-vector multiply ($O(N^2)$) for each time-step. In contrast, classic RBF collocation methods also construct LU factors of $A_{\diffop{}}^{-1}$ in a preprocessing, but delay application of forward- and back-solves to acquire time-dependent weighting coefficients at each time-step. This is then followed by the pre-multiply of $B$ (i.e., an additional $O(N^2)$) to complete the time-step.
 

%TODO: RBF-PS literature \cite{Fasshauer2006, Fasshauer2007}\cite{FasshauerZhang2007}\cite{WrightFlyerYuen10}\cite{Neves2009}\cite{FuselierWright2012}


\subsection{Local Methods}

Another trend in RBF methods is to use compact support to produce local linear systems defined at each collocation point. Each linear system then applies to a stencil of nodes around a center. Examples of this include \cite{Sarler2006, Vertnik2006} for Kansa's method, \cite{Stevens2008a, Stevens2009a, Stevens2009b} for Fasshauer's method. To our knowledge no one has considered local Direct collocation.  Also, instead of specifying a cut-off radius for RBF support, some authors specify the exact stencil size (i.e., number of neighboring points to include); see e.g., \cite{Divo2007, Stevens2009b}. In similar fashion, this work adopts the convention of specifying fixed stencil sizes.
...
\begin{eqnarray*}
u^{(k)}_\phi(x_i) & = & \sum_{j(k)=1}^{n_{I}}  c_j^{(k)} \diffop{\phi_j(x_i)} + \sum_{j(k)=n_{I} + 1}^{n} c^{(k)}_j\boundop{\phi_j(x_i)} + \sum_{l=1}^{M}d^{(k)}_l P_l(x_i)
\end{eqnarray*}
where $n$ represents the number of points that defines the local stencil (including the stencil center); $n$ is possibly a function of the cut-off radius in the RBF, $n_{I}$ is the number of interior stencil points (those points of the stencil that lie in the interior of $\Omega$). The index $j$ is a function of the stencil center $k$ allowing the system to include a local neighborhood of stencil points.
...
Most recently, Fornberg et al. \cite{Fornberg2012} introduced a new method called RBF-GA, which performs a similar change of basis as RBF-QR, but the method avoids truncated infinite expansions by expressing the new basis functions in terms of an incomplete Gamma function. Unlike RBF-QR, this method is limited to Gaussian basis functions only. Like RBF-QR, RBF-GA is also effective only for a small number of nodes: a few hundred in 2-D, and at least 500 in 3-D. Unlike RBF-QR, which performs a change of basis on the interpolating matrix only, the RBF-GA method requires a complicated change of basis for the RHS as well \cite{Fornberg2012}

Benchmarks provided in \cite{Fornberg2012} rank the stable methods for RBFs from fastest to slowest as: RBF-Direct, RBF-GA, and then RBF-QR. Keeping a perspective on run-times, the authors note that RBF-GA is roughly 10x slower than RBF-Direct and RBF-QR is another 2x-4x slower than RBF-GA \cite{Fornberg2012}.


%\part{RBF-FD for HPC Environments}

\chapter{Introduction to the RBF-FD Method}
...
Following \cite{FornbergLehto11}, weights for a 1-D classical-FD stencil can be obtained by solving a Vandermonde system,
	\begin{eqnarray}        
...
where the the $x_j$ are assumed to be distinct for guaranteed nonsingularity. In higher dimensions, the use of multivariate polynomials no longer guarantees nonsingularity of the matrix (which is no longer Vandermonde), so FD stencils are typically composed by adding weights from individual spatial directions. 

In contrast to Equation~\ref{eq:classical_fd_weight_system}, RBF-FD weights arise by enforcing that the weights exactly reproduce $\left. \diffop{\phi_i(x)} \ \right|_{x=x_c}$ within the space spanned by the RBFs that are centered at stencil nodes. This amounts to replacing each polynomial basis function $\{1, x, x^2, \cdots, x^{n-1}\}$ in Equation~\ref{eq:classical_fd_weight_system} with a $d$-dimensional RBF, $\phi_j(x)$, which allows for nonsingularity in $d$-dimensions on irregular node placements \cite{FornbergLehto11}. Various studies  \cite{WrightFornberg06,FornbergDriscoll02,FornbergLehto11,FlyerLehto11} show that better accuracy is achieved when the 
interpolant can exactly reproduce a constant, $p_0$, such that	
\begin{align*}
...
The small $(n + 1) \times (n + 1)$ system in Equation~\ref{eq:rbffd_weight_system} is dense, and is easily solved at a cost of $O(n^3)$ floating point operations (FLOPs) using direct methods like LU-decomposition. Here we refer to direct solution of Equation~\ref{eq:rbffd_weight_system} as RBF-Direct. Also, the assumption that nodes are static implies that producing the weights is a one-time cost. The resulting stencil weights, $c_\diffop{} = \{c_j\}_{j=1}^n$ can be substituted into Equation~\ref{eq:derivFromFDWeights} for the derivative approximation at $x_c$. Coefficient $c_{n+1}$ ($d_\diffop{} = c_{n+1}$), included in the solution of Equation~\ref{eq:rbffd_weight_system}, is of no use and discarded once the system has been solved. 

Based on the choice of support parameter, $\epsilon$, the Equation~\ref{eq:rbffd_weight_system} may suffer problems with conditioning. In such cases, stable methods for solving the system like Contour--Pad\'{e} \cite{Wright2003}, RBF-QR \cite{Fornberg2011a,Davydov2011}, or RBF-GA \cite{Fornberg2012} may be preferred to RBF-Direct.  
...
%TODO: expand these descriptions
%\authnote{Expand these reference descriptions} 
...
Shu et al. \cite{Shu2006} compared the RBF-FD method to Least Squares FD (LSFD) in context of 2-D incompressible viscous 
...
equation in \cite{Wright2004}.  Chandhini and Sanyasiraju \cite{Chandhini2007} studied it in the context of 1-D and 2-D, 
...
The small system solve in Equation~\ref{eq:rbffd_weight_system} or \ref{eq:multi_rhs_rbffd}  is repeated $N$ times---once for each stencil---to obtain a total of $N \times n$ stencil weights per differential operator. 
...
D_\diffop{}^{(c,k)} & = \begin{cases} c_j & x_k = x_j \\
 ...
 \end{cases} 
\end{align*}
where the condition $x_k = x_j$ maps the local stencil index for each node, $j$, to a global index, $k$, and $(c,k)$ is the global (row, column) index of $D_\diffop{}$. Vector $u = \{u(x_k)\}_{k=1}^{N}$. Equation~\ref{eq:derivFromFDWeights} can be rewritten as:
\begin{align*}
...
\end{align}
Note that the nonzeros in $D_{\diffop{}}$ are independent of the values in $u$. Approximating $\diffop{}$ over any function reduces to sampling the function values at nodes $\{x_k\}_{k=1}^N$ and performing a matrix-vector multiply. 

An example RBF-FD DM is illustrated in Figure~\ref{fig:example_DM_rows}. In this example, assume operator $\diffop{} = \pd{}{x}$ is approximated at all $N$ stencil centers of an arbitrary domain. RBF-FD weights assemble the rows of the differentiation matrix, $D_{x}$. On each row, weights are indicated by blue dots. The sparsity of rows reflects the subset of $\{x_k\}_{k=1}^N$ included in corresponding stencils of size $n$. The mapping between global indices ($k$) and local indices ($j$) is used for each sum. On the right hand side, discrete derivative values $\d{u}{x}$ are approximated at all stencil centers. 
...
Differentiation matrices are assembled at a cost of $O(n^3 N)$ FLOPs. %However, since the goal of RBF-FD is to keep stencils small ($n \ll N$), the cost of assembly scales as $O(N)$. 
Furthermore, since RBF-FD weights are only a function of node locations, the stencil weights are constant so long as the nodes are stationary. Thus, the DM assembly is part of a one-time preprocessing step.% and can applied explicitly in $O(N)$ operations  

For linear PDEs one often assembles a single DM to represent the operator for the entire differential equation, but RBF-FD allows flexibility in how operators are handled. Rather than a single DM, with weights from a new operator on the RHS of Equation~\ref{eq:multi_rhs_rbffd}, one may approximate the operator based on lower order derivatives. Consider for example, the 2-D Laplacian operator, $\Laplacian = \pdd{}{x} + \pdd{}{y} $: 
\begin{align*}
...
In Chapters~\ref{chap:stencils} and \ref{chap:distributed_rbffd} the concept of an \emph{adjacency matrix} is required. Adjacency matrices have the same structure as a DM, except that all nonzero elements are masked by the value 1. Nonzero elements indicate node connectivity and adjacency graph. The \emph{degree} of a node is the number of off-diagonal nonzeros in the adjacency matrix. Equivalently, the degree is the number of connections to neighboring nodes in the adjacency graph. For example, Figure~\ref{fig:directed_graph} shows four nodes connected via directional arrows. Arrows originate at a stencil center and terminate at stencil nodes. Both the graph and matrix indicate that $B$ depends on $A$, $C$ and $D$, while $C$ depends on $B$, and $D$ depends on $A$. $A$ has no dependencies. The degree for each node is given as zero for $A$, three for $B$, one for $C$ and one for $D$. The graph in Figure~\ref{fig:directed_graph} is directed with an associated unsymmetric adjacency matrix. Figure~\ref{fig:undirected_graph} demonstrates the case where edges are bijective resulting in an undirected graph and symmetric adjacency matrix. 

Some operations, e.g., matrix decompositions and matrix reordering, are easier when applied to symmetric adjacency matrices. RBF-FD can benefit from some of those same operations by symmetrizing the adjacency matrix $A$ with $A+A^T$. Note that all nonzeros in $A+A^T$ continue to be masked by ones.
...
\subsection{First and Second Derivatives (\texorpdfstring{$\frac{1}{r}\pd{\phi}{r}, \pdd{\phi}{r}$}{ddx phi})}
The following are used in subsequent dervatives:
\begin{align}
\frac{1}{r}\d{}{r}\phi(r) & = -2 \epsilon^2 \phi(r) \label{eq:dphi_dr} \\
\pdd{\phi}{r} & = \epsilon^2(-2 + 4(\epsilon r)^2)\phi(r) \label{eq:dphi2_dr2}
\end{align}



\subsection{Cartesian Gradient (\texorpdfstring{$\grad$}{nabla})}
...
%	 \pd{\phi}{z} = \pd{r}{z} \pd{\phi}{r} = \frac{(z-z_{j})}{r} \pd{\phi}{r}
	\end{align*}
where $\pd{\phi}{r}$ for the Gaussian RBFs is given in Equation~\ref{eq:dphi_dr}. Formulas are similar for $\pd{}{y}, \pd{}{z}$ (substitute on $x$). 


\subsection{Cartesian Laplacian (\texorpdfstring{$\Laplacian$}{nabla2})}
The general form of $\nabla^2$ in $d$-dimensions is  
\begin{align*}
\nabla^2 \phi = \pdd{}{r}\phi(r) + \frac{d - 1}{r}\pd{}{r} \phi(r)
\end{align*}
which depends on Equation~\ref{eq:dphi_dr} and \ref{eq:dphi2_dr2}. 
For Gaussian RBFs in particular we have the following operator:
\begin{align}
\nabla^2 \phi = 2 \epsilon^2 (-d+2 (\epsilon r)^2) \phi(r)
\end{align}
%\begin{itemize}
...
\subsection{Laplace-Beltrami (\texorpdfstring{$\LaplaceBeltrami$}{Delta s}) on the Sphere}

The $\Laplacian$ operator can be represented in spherical polar coordinates for $\mathbb{R}^3$ as: 
\begin{align*} 
\Laplacian \phi = \left(\underbrace{\frac{1}{r} \pd{}{r} \left( r^{2} \pd{}{r}  \right)}_{\mathsf{radial}} + \underbrace{\frac{1}{r^2} \Delta_{S}}_{\mathsf{angular}} \right) \phi , %\label{eq:laplacian_in_spherical}
\end{align*}
...
In the case of PDEs solved on the surface of unit sphere, there is no radial term, resulting in:
\begin{align*}
...
\begin{align*} 
\LaplaceBeltrami \phi = \frac{1}{4} \left[ \left(4-r^2\right) \pdd{\phi}{r} + \frac{4-3r^2}{r} \pd{\phi}{r} \right],
\end{align*} 
...
\subsection{Constrained Gradient (\texorpdfstring{$P_{x} \cdot \grad$}{P dot nabla}) on the Sphere}
\label{sec:projected_grad}

Following \cite{FlyerWright09, FlyerLehto11}, the gradient operator can be constrained to the sphere with the following projection matrix: 
%\frac{1}{||\mathbf{x}||}
\begin{align}
...
Using the chain rule, and the assumption that 
\begin{equation*}
r(\vx_k-\vx)=\vectornorm{\vx_k-\vx} = \sqrt{(x_{1,k}-x_1)^2 + (x_{2,k}-x_2)^2 + (x_{3,k}-x_3)^2},
\end{equation*}
 the unprojected gradient of $\phi$ becomes
\begin{align*}
\nabla \phi(r(\vx_k - \vx)) = \nabla r  \pd{}{r} \phi(r(\vx_k - \vx)) = - (\vx_k - \vx)\frac{1}{r(\vx_k - \vx)} \pd{}{r} \phi(r(\vx_k - \vx)) .
\end{align*} 
...
\subsection{Hyperviscosity (\texorpdfstring{$\Delta^k$}{Delta k}) for Stabilization}
...
%However,
%numerical evidence in \cite{FornbergLehto11,BolligFlyer} \ci suggests that if the RBFs adequately resolve the convective operator, the 2-D 
...
Applying hyperviscosity shifts all the eigenvalues of $L$ (the discrete form of $\diffop{}$). With the correct sign and careful choice of scaling, hyperviscosity is able to shift all eigenvalues to the left half of the complex plane. 
...
This section considers at a high level how one leverages RBF-FD to solve PDEs. To simplify the discussion, consult Algorithm~\ref{alg:rbffd_high_level}, which is split into two phases: Preprocessing and Application. The complexity for each phase can vary based on the algorithms utilized in each task. 
...
\begin{algorithmic}[0]                   
    \State $\{x\}_{j=1}^N$ = {\sc GenerateGrid}()
   	\For {$j = 1$ to $N$} %\label{alg:stencil_gen} 
   	    \State Stencil $\{S_{j,i}\}_{i=1}^{n}$ = {\sc QueryNeighbors}($x_j$)
   	\EndFor
    \State $\{x\}_{j=1}^{N_p} = $ {\sc DecomposeDomain}($D_{\diffop{}}$, $\{x\}_{j=1}^N$)
    \For {$j = 1$ to $N_p$} %\label{alg:weight_calc} 
   	    \State $\{w_{j,i}\}_{i=1}^{n} = $ {\sc SolveForWeights}($\{S_j\}$)
   	    \State $D_{\diffop{}}^{(j)} = $ {\sc AssembleDM}($\{w_j\}$)     
    \EndFor
    \algstore{myalg}    % Store the algorithm number count
\end{algorithmic}
\textbf{Application Phase:}
\begin{algorithmic}[1] 
\algrestore{myalg}      % continue to count from store line
    \State $t = t_{min}$                
    \While{ $t < t_{max}$ }
        \State $\{u'\} =$ {\sc SolvePDE}($D_{\diffop{}}$, $\{u\}$)
        \State $\{u\} =$ {\sc UpdateSolution}($\{u\}$, $\{u'\}$, $\Delta t$)
        \State $t$ += $\Delta t$
    \EndWhile
\end{algorithmic}
\end{algorithm}

The algorithm loads/generates a grid. RBF-FD requires only node coordinates, and in some cases an indication of whether nodes are on a boundary. Information on mesh edges/connectivity is optional, but could be used to bypass stencil generation. The \textsc{QueryNeighbors} step forms stencils and constructs a directed adjacency matrix that indicates the connectivity of stencils. Either the grid or the adjacency matrix can be partitioned (see Chapter~\ref{chap:distributed_rbffd}) for distribution across multiple processors in the \textsc{DecomposeDomain} step. Finally, one or more processors operate independently to compute weights and assemble local DMs. The assumption in this work is that grids do not evolve in time, so DMs remain constant for the duration of the Application phase. In the event of moving nodes, weight calculation and DM assembly would move into the loop in the second phase. Note however, that the cost of DM generation would be excessive if the grid changed at every iteration. Efficiently handling grids with moving nodes and/or changing connectivity is a subject of future research. 



Constructed DMs are applied to solve a PDE either explicitly (e.g., Equation~\ref{eq:explicit_dm}) or implicitly (e.g., Equation~\ref{eq:implicit_dm}). In the case of time-dependent PDEs, RBF-FD is applied similarly to any classical FD method with a time-stepping scheme (represented by \textsc{UpdateSolution}). Examples of valid time-schemes include Runge-Kutta, Adams-Bashforth, and Crank Nicholson methods among many others. Based on the choice of time-scheme, one frequently needs multiple iterations through \textsc{SolvePDE} to assemble intermediate solutions that are weighted and combined in the \textsc{UpdateSolution} step.

%TODO: add refs for RBF-FD uses of these schemes. 
%TODO: add CFL ref

Generally the performance of RBF-FD hinges on the cost of \textsc{SolvePDE}. This is especially true for time-dependent PDEs where the CFL condition relates the number of required time-steps to the spacing between nodes. As the problem size grows, smaller time-steps must be taken in order to maintain stability. Chapter~\ref{chap:stencils} demonstrates that choosing the right algorithms for preprocessing tasks can also directly impact the performance of SpMV. Beyond this, the choice of stencils and accuracy of weights can impact the overall accuracy of approximation, stability and conditioning of the method. As each of these properties improve, the overall time to solution can decrease thanks to larger stable time-steps, coarser grids, and faster convergence.


\section{Grids} 


RBF-FD has no requirement for structured grids, nor a need for a well-refined mesh/lattice to define and limit connectivity between nodes. It functions the same on existing meshes and randomly generated point clouds, although the actual node placement can not only impact accuracy of the method, but stability as well. The only restriction from the method is that coincident nodes should not be connected. This functional portability on domains of any shape, dimension, and granularity is a major selling point, and something often out of reach for other methods. 

For benchmarking purposes we test a number of \emph{grid refinements}. In this context, a refinement is assumed to be a uniform increase in the global problem size. It is possible, but not required, for refinement to be a \emph{subdivision} of a coarse grid, which implies the coarse grid is included as a proper subset of the finer grid.
%\begin{itemize} 
...
\subsubsection{Regular Grid}For basic debugging and benchmarking purposes the most natural choice is to start with a regular or Cartesian grid. Equally spaced nodes in multiple dimensions are simple to generate. Additionally, grid refinements---for scaling benchmarks and convergence tests---can be generated as subdivisions of coarser grids (i.e., the coarser grids are subsets of the fine grids). 

In theory, RBF-FD functions the same whether nodes are uniformly spaced or random. However, regular grids do not fully expose advantages that RBF-FD has over other methods with its ability to operate on scattered nodes. For this reason regular grids are only used here for benchmarking purposes and range in size from $N=10^3$ to $N=160^3 =$ 4,096,000 nodes. 



\subsubsection{Maximum Determinant Nodes}

Chapter~\ref{chap:applications} applies RBF-FD to solve PDEs on the unit sphere. For consistency with respect to related investigations (e.g., \cite{Fornberg2009a, Fornberg2007,FornbergLehto11}), the Maximum Determinant (MD) node sets \cite{Womersley2001, Sloan2003} are chosen. 
...
%GORDON: How does this show irregularity? You are not referring to the virtual boundaries, but to the flat region in the center? It is hard to see the irregularities due to the curvature of the sphere. You could have created histrams of nearest neighbor distance or 2nd neighbor distance. I did this in 3-D for the work I am doing on random 3-D generation with Bengt and Natasha. Bengt gave me the idea. I have a Python implemenation.
...
%GORDON: How long does convergence take with the largest grid you have (say one million)? {will need to contact Geoff about that}




\section{On Choosing the Right \texorpdfstring{$\epsilon$}{e}} 
...
If solving for RBF-Direct weights directly (i.e., inverting Equation~\ref{eq:rbffd_weight_system} directly), one must balance the choice of $\epsilon$ against ill-conditioning to achieve a reasonable accuracy for the weights. %Numerous attempts exist in the literature to provide ``good" functions for $\epsilon$ based on node spacing ($h$), stencil size ($n$), and total number of nodes in the domain ($N$). 

Unfortunately, no foolproof method exists to scale $\epsilon$ as a function of $N$ for RBF-Direct---the ``optimal" (in most cases this reads: ``most acceptable") value of $\epsilon$ depends on the node distribution and varies by application. 
As a rule of thumb, when the spacing between nodes decreases $\epsilon$ must increase. 
Parameter sweeps to characterize the behavior of $\epsilon$ on large problems can be prohibitively expensive in terms of time and/or money. Ultimately, stable methods for obtaining weights (see \S~\ref{sec:stable_methods}) are really necessary for general large-scale applications, even if such methods are 10x--40x slower than RBF-Direct \cite{Fornberg2012}. 

%GORDON: On large problems that are expensive, how is epsilon chosen? You might not have the $$ to do long, expensive test runs. 
% Where is kappabar = 1/N … first defined? Same without the bar. Defined where?
% Use computer font for cond()

This work utilizes a moderately reliable method, proposed in \cite{FlyerLehto11}, to choose $\epsilon$ as a function of the grid resolution, $N$. The method performs a parameter sweep in $\epsilon$ and $N$ to generate a set of mean condition numbers, $\bar{\kappa}_A$, defined as:
\begin{align*}
\bar{\kappa}_A = \frac{1}{N} \sum_{i=1}^N(\kappa_A)_i
\end{align*}
where $(\kappa_A)_i$ is the condition number for the matrix, $A_i$, assembled by Equation~\ref{eq:rbffd_weight_system} for the $i$-th stencil in the domain. MATLAB's \textsc{cond} command computes the 2-norm condition number defined as:
\begin{align*}
(\kappa_A)_i = ||A_i^{-1}||_{2}\cdot||A_i||_{2}.
\end{align*} 

 %Here $\kappa_A$ is the condition number of the interpolation matrix from Equation~\ref{eq:rbffd_weight_system}. 

The tiles of Figure~\ref{fig:epsilon_contours} illustrate a number of contours generated in MATLAB. Each contour is numbered according to $\log_{10}\bar{\kappa}_A$. Data was generated by uniformly sampling parameter spaces on $\epsilon$ and $\sqrt{N}$ for the MD node-sets. For each $\sqrt{N} = \{40, ..., 100\}$, in steps of 10, the code makes a sweep through $\epsilon = \{1, ..., 10\}$ in steps of 0.5 and assembles $N$ RBF-FD interpolation matrices. The \textsc{cond} command evaluates $(\kappa_A)_i$ for each matrix. At the end of the sweep, the 2-D array of mean condition numbers are passed through the MATLAB \textsc{contour} command to identify and trace the line segments.

The resulting values of $\bar{\kappa}_A$ produce remarkably linear contours, with slopes that fan out from the $\epsilon$ axis. Note that the $(\kappa_A)_i$ depend on the stencil size, $n$, as evidenced by shifting contour fans for $n=20, 40, 60, 80,$ and $100$. For large $n$ we observe that the simultaneous increase in slope and decreasing separation between contours leaves little ``wiggle" room in guessing $\epsilon$ under RBF-Direct. 
...
Stable weight algorithms like Contour-Pad\'{e} \cite{Wright2003}, RBF-QR \cite{Fornberg2007, Fornberg2011a, Davydov2011}, and RBF-GA \cite{Fornberg2012} can allow RBF-FD to scale beyond the limits of RBF-Direct and will be included in a future investigation. 
...
%\section{Metrics} 
%
%The following chapters consider the parallelization of RBF-FD on GPUs, across multiple CPUs, and across multiple GPUs. In each case we assert that the code produces the same results. However, the time to solution differs, so we make use of the speedup metric: 
%$$ 
%S_{p} =\ ^{T_{\text{serial}}} /_{T_{\text{parallel}}}
%$$
%to monitor improvement in performance.
%
%\authnote{finish}

%TODO: \authnote{Include efficiency}: 
%$$ 
