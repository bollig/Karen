...
The small $(n + 1) \times (n + 1)$ system in Equation~\ref{eq:rbffd_weight_system} is solved at a cost of $O(n^3)$ operations. The resulting stencil weights, $c_\diffop{} = \{c_j\}_{j=1}^n$ can be substituted into Equation~\ref{eq:derivFromFDWeights} for the derivative approximation at $x_c$; coefficient $c_{n+1}$, included in the solution of Equation~\ref{eq:rbffd_weight_system}, is of no use and discarded. 

\authnote{HERE} 
Equation~\ref{eq:rbffd_weight_system} resolves the stencil weights for one node in the domain, $x_c$. The process is repeated $N$ times---once for each stencil center in the domain---to obtain a total of $N \times n$ weights


Approximating $\diffop{u(x)}$ for all nodes in the domain, $\{x_c\}_{c=1}^N$, requires iterating Equation~\ref{eq:rbffd_weight_system} must be solved $N$ 

This process is repeated for each stencil center  $\{x_c\}_{c=1}^{N}$ for a total of $N \times n$ weights. 
...
\label{sec:rbffd}
 ...
 $\phi_i(\epsilon r) = \phi(\ep\vectornorm{\vx-\vx_{i}})$, centered at the nodes $\{\vx_i\}_{i=1}^{n}$, with 
$r=\vectornorm{\vx - \vx_i}$
being the distance between where the RBF is centered and where it is evaluated as measured in the standard Euclidean 2-norm.
