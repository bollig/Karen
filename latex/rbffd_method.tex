...
\underbrace{\begin{bmatrix} \phi & P \\
		P^T & 0 \end{bmatrix}}_{A} \begin{pmatrix} c_\diffop{} \\ 
							d_\diffop{} \end{pmatrix} & = & \underbrace{\begin{pmatrix} \phi_{\diffop{}} \\
							0 \end{pmatrix}}_{B_{\diffop{}}}. \nonumber
	\end{eqnarray}	

The small $(n + 1) \times (n + 1)$ system in Equation~\ref{eq:rbffd_weight_system} is solved at a cost of $O(n^3)$ floating point operations (FLOPs). The resulting stencil weights, $c_\diffop{} = \{c_j\}_{j=1}^n$ can be substituted into Equation~\ref{eq:derivFromFDWeights} for the derivative approximation at $x_c$. Coefficient $c_{n+1}$ ($d_\diffop{} = c_{n+1}$), included in the solution of Equation~\ref{eq:rbffd_weight_system}, is of no use and discarded once the system is solved. 

Note that Equation~\ref{eq:rbffd_weight_system} resolves the weights for one node in the domain, $x_c$.  Thus, the small system solve is repeated $N$ times---once for each node---to obtain a total of $N \times n$ stencil weights.
...
\subsubsection{Differentiation Matrix}
to form the $N$ rows of the DM  with $n$ non-zeros $(n\ll N)$ per row.
...
differentiation weights is performed once in a single preprocessing step of O$(n^3N)$ FLOPs.


\subsubsection{Multiple Derivatives}

In many cases, multiple differential operators (i.e., $\diffop{}_1, \diffop{}_2, \cdots$) are required


%TODO: multiple RHS
\begin{eqnarray}
\underbrace{\begin{bmatrix} \phi & P \\
		P^T & 0 \end{bmatrix}}_{A} \begin{pmatrix} c_{\nabla^2} & c_{\pd{}{x}} & \cdots \\ 
							d_{\nabla^2} & d_{\pd{}{x}} & \dots \end{pmatrix} & = & \underbrace{\begin{pmatrix} \phi_{\nabla^2} & \phi{\pd{}{x}} & \cdots \\
							0 & 0 & \cdots \end{pmatrix}}_{B_{\diffop{}}}. \nonumber
	\end{eqnarray}	
