...
The constructed DMs are applied in the Application phase to solve the PDE either explicitly or implicitly. For explicit solutions, this involves evaluating a Matrix-Vector multiply within the algorithmic step denoted as SolvePDE. In the case of implicit solutions, SolvePDE must invert the DM via a direct or iterative linear system solve. Although it is not mandatory, many PDE solutions are time-dependent and require some time advancement scheme. The time-scheme could be as simple as an Euler update with a single call to SolvePDE. Higher order methods (e.g., Runge-Kutta, Adams-Bashforth, etc.) may require multiple evaluations of SolvePDE, with the outputs weighted and combined within UpdateSolution. 
