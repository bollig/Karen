...
\authnote{RBF-FD nodes should not coincide}
...
\subsubsection{Regular Grid}For basic debugging and benchmarking purposes the most natural choice is to start with a regular or Cartesian grid. Equally spaced nodes in 1D, 2D or 3D are simple to generate. Additionally, refinements (for convergence tests) are direct subsamples. RBF-FD functions the same whether nodes are uniformly spaced or random. From this perspective, one can argue that regular grids do not fully exercise the advantages from RBF-FD. 

\subsubsection{Maximum Determinant Nodes}

In Chapter~\ref{chap:explicit_implicit_pdes} we verify our RBF-FD implementation by solving PDEs on the unit sphere. Following the lead of many others in the RBF literature \cite{FlyerWright07,FlyerLehto10}, %TODO: need refs
we test our solutions using the Maximum Determinant node sets. 

For most tests on sphere we load MD node sets to confirm results with other RBF literature
