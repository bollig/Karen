...
This section considers at a high level how one leverages RBF-FD to solve PDEs. To simplify in the discussion, consult Algorithm~\ref{alg:rbffd_high_level}, which is split into two phases: preprocessing and application. The complexity for each phase can vary based on the algorithms utilized in each task. 
...
%GORDON: How does this show irregularity? You are not referring to the virtual boundaries, but to the flat region in the center? It is hard to see the irregularities due to the curvature of the sphere. You could have created histrams of nearest neighbor distance or 2nd neighbor distance. I did this in 3D for the work I am doing on random 3D generation with Bengt and Natasha. Bengt gave me the idea. I have a Python implemenation.
...
%TODO: How do plots change if nodes are uniform in a cube or if 2D, on a square? What is the effect of boundaries? Are there any? Boundaries (like for FD), would reduce the Courant number since points would probably be closer to each other.
