...
The RBF-PS method was introduced by Fasshauer \cite{Fasshauer2006, Fasshauer2007}. 

The RBF-PS applies the same logic as Kansa's and Fasshauer's collocation methods to form matrix $A_{\diffop{}}$. However, RBF-PS makes a subtle change to assume the solution, $u(x)$, is only required at collocation points \cite{Fasshauer2006, Fasshauer2007}. Then, extending Equation~\ref{eqn:solve_u} for the discrete differentiation matrix, $D_\diffop{}$, gives:
\begin{eqnarray}
...
\end{eqnarray}
Here, $D_\diffop{}$ is independent of the function $f(x)$ and is assembled by solving the multiple RHS linear system: 
\begin{eqnarray}
D_{\diffop{}} & = & A^{-T}_{\diffop{}} B^T
\end{eqnarray}
For assembly, an LU-decomposition can be used ($O(N^3)$) in preprocessing \cite{WrightFlyerYuen10,Fasshauer2007}. Forward- and back-solves for the decomposition are applied immediately ($O(N^2)$) to complete assembly. 

Since matrix $D_{\diffop{}}$ is independent of $u(x)$ and $f(x)$, it is only updated if RBF centers move---a compelling benefit for time-dependent problems with stationary nodes---otherwise the solution is given by an $O(N^2)$ matrix multiply. In contrast, global RBF collocation methods construct the LU factors of $A_{\diffop{}}$ in preprocessing, but must apply forward- and back-solves at each time-step ($O(N^2)$) to obtain weighting coefficients before the pre-multiply of $B$ (additional $O(N^2)$ step) provides the solution.
 
%TODO: RBF-PS \cite{Fasshauer2006, Fasshauer2007}\cite{FasshauerZhang2007}\cite{WrightFlyerYuen10}

%When the test points coincide with the trial points we have the RBF-PS method. 
