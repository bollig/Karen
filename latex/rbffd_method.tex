...
MD node sets were originally introduced to the RBF community due to demonstrated success in spherical harmonics interpolation on the sphere. The seemingly irregular distribution of nodes gain an order of magnitude accuracy over regular looking node distributions \cite{Womersley2001}. Since RBF interpolation tends to reproduce spherical harmonics interpolants on the sphere when $\epsilon \rightarrow 0$, RBF methods benefit similarly \cite{Fornberg2009a}. 

MD node files are available for download on the authors' web site (\url{http://web.maths.unsw.edu.au/~rsw/Sphere}), and range in size from $N=4$ up to $N=27,556$ nodes. Figure~\ref{fig:md_nodes} plots the $N=4096$ MD node set to illustrate the irregularity in node distribution. 
