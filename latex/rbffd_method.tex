...
No foolproof method exists for RBF-Direct---the ``optimal" (in most cases this reads: ``most acceptable") value of $\epsilon$ depends on the node distribution and varies by application. That said, parameter sweeps on large problems are expensive, so finding the right function for $\epsilon$ 
