...
In the Application phase, constructed/loaded DMs are applied to known solution values to solve the PDE either explicitly or implicitly. Although it is not mandatory, Algorithm~\ref{alg:rbffd_high_level} assumes that this phase includes a form of time-stepping. Time-stepping could be as simple as an Euler method ($u_{new} = u + \Delta t u'$)
or more complex like Runge-Kutta where SolvePDE would be called for multiple steps and the results of each call would be weighted and combined within UpdateSolution.

 As the number of time-steps increases, the cost of Application far outweighs the one-time cost of preprocessing; thus, this work focuses mostly on acceleration of Application where the impact of gains will be most significant. 
...
\textbf{Preprocessing:}
\begin{algorithmic}[0]                   
    \State Load or construct grid $\{x\}_{j=1}^N$ 
...
\algrestore{myalg}      % continue to count from store line
    ...
    \While{ $t < t_{max}$ }
        ...
        \State $\{u\} =$ UpdateSolution($\{u'\}$, $\Delta t$)
        \State $t$ += $\Delta t$
    \EndWhile
\end{algorithmic}
\end{algorithm}
