...
%TODO: direct solve vs compose DM for operators
...
This section considers at a high level how one implements RBF-FD to solve PDEs. To simplify in the discussion, consult Algorithm~\ref{alg:rbffd_high_level}, which is split into two phases: preprocessing and application. The complexity of each phase can vary based the algorithms utilized for each task. 

Preprocessing encompasses tasks such as grid setup/generation, stencil generation and stencil weight calculations. As output from these tasks one expects one or more DMs representing the discrete differential operators. Assuming the grids do not evolve in time, the DMs remain constant for the duration of the second phase; otherwise, the second loop would join the second phase. Furthermore, both grids and DMs can be loaded from disk on subsequent runs to effectively bypass the cost of all preprocessing. 
...
In the Application phase, the constructed DMs are used to solve a PDE either explicitly (Equation~\ref{eq:explicit_dm}) or implicitly (Equation~\ref{eq:implicit_dm}). 
%TODO: FiX
\authnote{Fix:} Note that regardless of the explicit or implicit method, this phase reduces to a Sparse Matrix-Vector Multiply (SpMV) within the SolvePDE step. For explicit solutions, SolvePDE computes the SpMV in Equation~\ref{eq:explicit_eq}. In the implicit case, SolvePDE must invert the DM in Equation~\ref{eq:implicit_eq} via a direct or iterative linear system solve. In the case of time-dependent PDEs, RBF-FD is applied the same as any other finite difference method within a time-stepping scheme (represented by UpdateSolution). % (e.g., Runge-Kutta, Adams-Bashforth, etc.) may require multiple applications of the DM, with the outputs weighted and combined within UpdateSolution. 
