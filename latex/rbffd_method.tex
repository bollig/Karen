...
SCVTs come with a sense of "optimality" in node locations due to their energy minimizing properties \cite{Du2003b}. Generating SCVTs involves constructing a Voronoi diagram, computing the mass centroids for each Voronoi partition, and updating generating node locations. In most large-scale applications, this iterative process is completed via a probabilistic Lloyd's method, where integrals to compute mass centroids are approximated through random sampling \cite{Womeldorff2008, Du2003b}. While SCVTs in theory converge to a regular node distribution, the probabilistic nature of the centroid calculation introduces irregularities in the grids reminiscent of MD nodes. 

Figure~\ref{fig:scvt_nodes} provides an example SCVT grid with N=100,000 nodes. On the left, the full sphere; on the right, a close-up of the same node set. The close-up perspective clearly demonstrates the random artifacts/scarring of irregularly distributed nodes. For benchmarking purposes we test N=100,000, N=500,000 and N=1,000,000 node sets generated by the SCVT library from \cite{Womeldorff2008}. 
