...
Following \cite{FornbergLehto11}, weights for a 1D classical stencil can be obtained by solving a Vandermode system
	\begin{eqnarray}        
...
The sparsity exhibited by the DM in Figure~\ref{fig:example_DM_rows} is typical for RBF-FD. Consider that a problem size of $N=10,000$ and $n=31$ is only $0.31\%$ full, and the percentage continues to decrease as $N$ increases. 
Best practices dictate that the DMs be stored in some type of compressed sparse storage that retains only non-zeros and their corresponding indices in memory. In Chapter~\ref{chap:gpu_rbffd} the choice of sparse storage plays a pivotal role in the performance of RBF-FD. 
...
% TODO: High level algorithm should lay out: grid gen/load, stencil gen, weight gen, domain decomposition, assemble PDE, solve PDE and update solution. 
% TODO: no details on 
This section provides an overview of how one implements RBF-FD. Consider Algorithm~\ref{alg:rbffd_high_level}, which is has two phases: preprocessing and application. The complexity of each phase depends on the choice of algorithms/method utilized for each task. 
...
\authnote{perhaps this belongs in an appendix}
...
\includegraphics[width=9.5cm]{../figures/stencils/sphere_stencil_gen_speedup.png}
\caption{Querying the $n=50$ nearest neighbors on a regular grid up to $N=160^3$ demonstrates the significant gains achieved by our spatially binned neighbor query. While KDTree queries grow as $O(N log N)$}
\label{fig:hash_results}
\end{figure}
\begin{figure}
...
\begin{figure}
\centering
\includegraphics[width=9.5cm]{../figures/stencils/sphere_spmv_speedup.png} 
\caption{Querying the $n=50$ nearest neighbors on a regular grid up to $N=160^3$ demonstrates the significant gains achieved by our spatially binned neighbor query. While KDTree queries grow as $O(N log N)$}
\label{fig:hash_results}
\end{figure}
