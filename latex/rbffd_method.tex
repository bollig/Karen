...
After a grid is either generated or loaded stencils must be generated by querying the $n$ nearest neighbors to each $\{ x_j \}_{j=1}^N$. Brute force searching for neighbors---computing the distance between every pair of nodes and then selecting the $n$ nearest---is discouraged due to its $O(N^2)$ complexity. Alternatively, it is common practice in the RBF community to construct $k$D-Trees to decrease the cost of queries (e.g., \cite{Fasshauer2007, FlyerLehto11, FornbergLehto11}). 
