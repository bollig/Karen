...
Here, we present the first results in the literature related to the parallelization of the RBF-FDs methodology on multi-CPU and multi-GPU architectures with the goal of solving PDEs. %A less verbose version of this chapter was included in \cite{BolligFlyerErlebacher2012}. 
...
%The goal when choosing parameters is to push all the eigenvalues to the left half-plane, and then tweak $\gamma_c$ up or down to condense the eigenvalues as near to the imaginary axis as possible. 
...
Rather than merge the differentiation matrices in Equation~\ref{eq:cosine_with_hyperviscosity} into a single operator, our implementation evaluates them as two sparse matrix-vector multiplications. The separate operations are motivated by an effort to provide general and reusable GPU kernels. Note that they artificially increase the amount of computation compared to the vortex roll-up test case to simulate cases when operators cannot be merged into one DM (e.g., a non-linear PDE).
...
We advect a $C^1$ cosine bell height-field given by
\begin{equation*}
h  =
\begin{cases}
...
\end{subfigure}
\caption{Eigenvalues of Equation~\ref{eq:cosine_with_hyperviscosity} for the cosine bell test case with $N=4096$ nodes, stencil size $n=101$, and $\epsilon = 3.5$. Eigenvalues are divided by $u_0$ to remove the scaling effects of velocity.}
